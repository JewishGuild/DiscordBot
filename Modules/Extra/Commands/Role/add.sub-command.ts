...
class AddSubCommand extends BaseSubCommand {
  public async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    /* Add the role */
    if (member.roles.cache.has(role)) throw new Error(`Member already has the <@&${role}> role`);
  ...
  private constructEmbed(memberId: Snowflake, roleId: Snowflake) {
    return new Embed({ description: `Added <@&${roleId}> to <@${memberId}> successfully` });
  }

  protected buildData(): SlashCommandSubcommandBuilder {
    return new SlashCommandSubcommandBuilder()
      .setName("add")
      .setDescription("Adds special role")
